...
import { getClients, deleteClient, getInspections } from '@/lib/supabase'
...
export default function ClientsPage() {
  ...
  const [filter, setFilter] = useState<'all' | 'portal' | 'no-portal'>('all')

  useEffect(() => {
    Promise.all([getClients(), getInspections()])
      .then(([cls, insps]) => {
        setClients(cls)
  ...
  const portalCount = clients.filter(c => c.portal_password).length
  const withPropertiesCount = clients.filter(c => c.properties?.trim()).length

  // Count inspections per client by name match
  function inspectionCount(clientName: string) {
    return inspections.filter(i => i.client_name === clientName).length
  }
  ...
  const filtered = useMemo(() => {
    return clients.filter(c => {
      ...
      const matchFilter =
        filter === 'all' ||
        (filter === 'portal' && !!c.portal_password) ||
        (filter === 'no-portal' && !c.portal_password)
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        <StatCard
          ...
          sub={`${inspections.length} inspections`}
        ...
        <StatCard
          label="Portal Access"
          value={portalCount}
          sub={clients.length ? `${Math.round(portalCount / clients.length * 100)}% enabled` : '—'}
          color="bg-green-100 dark:bg-green-900/40 text-green-600 dark:text-green-400"
          icon={<svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.8} d="M9 12l2 2 4-4m5.618-4.016A11.955 11.955 0 0112 2.944a11.955 11.955 0 01-8.618 3.04A12.02 12.02 0 003 9c0 5.591 3.824 10.29 9 11.622 5.176-1.332 9-6.03 9-11.622 0-1.042-.133-2.052-.382-3.016z" /></svg>}
        />
        <StatCard
          label="With Properties"
          value={withPropertiesCount}
          sub="properties on file"
        ...
        <StatCard
          label="No Portal Yet"
          value={clients.length - portalCount}
          sub="invite pending"
      ...
      {loading ? (
        ...
        <div className="text-center py-16 bg-white dark:bg-slate-800 rounded-2xl border border-slate-200 dark:border-slate-700">
          ...
          <p className="text-slate-500 dark:text-slate-400 font-medium">No clients match your search</p>
        ...
        <div className="space-y-3">
          {filtered.map((client) => {
            ...
            const inspsForClient = inspectionCount(client.name)
            const hasPortal = !!client.portal_password
            const borderColor = hasPortal
              ? 'border-l-green-500'
            ...
            return (
              <div key={client.id}
                ...
                {/* Avatar */}
                <div className={`w-11 h-11 rounded-xl flex items-center justify-center flex-shrink-0 font-bold text-base ${avatarColor}`}>
                  {initials}
                </div>
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2 flex-wrap">
                    ...
                    {hasPortal && (
                      <span className="text-xs font-semibold px-2 py-0.5 rounded-full bg-green-100 dark:bg-green-900/40 text-green-700 dark:text-green-300 border border-green-200 dark:border-green-800/50 flex items-center gap-1">
                        <svg className="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 12l2 2 4-4m5.618-4.016A11.955 11.955 0 0112 2.944a11.955 11.955 0 01-8.618 3.04A12.02 12.02 0 003 9c0 5.591 3.824 10.29 9 11.622 5.176-1.332 9-6.03 9-11.622 0-1.042-.133-2.052-.382-3.016z" /></svg>
                        Portal
                      </span>
                    )}
                    {inspsForClient > 0 && (
                      <span className="text-xs font-medium px-2 py-0.5 rounded-full bg-blue-50 dark:bg-blue-900/30 text-blue-600 dark:text-blue-400 border border-blue-100 dark:border-blue-800/50">
                        {inspsForClient} inspection{inspsForClient > 1 ? 's' : ''}
                      </span>
                    )}
                  </div>
                  ...
                  {properties.length > 0 && (
                    <p className="text-xs text-slate-400 mt-1.5 flex items-center gap-1">
                      ...
                      {properties[0]}
                      {properties.length > 1 && (
                        <span className="text-slate-300 dark:text-slate-600"> +{properties.length - 1} more</span>
                      )}
                    </p>
                  )}
                </div>
...
function EmptyState({ t }: { t: { noClientsYet: string; createFirstClient: string; addClient: string } }) {
  return (
